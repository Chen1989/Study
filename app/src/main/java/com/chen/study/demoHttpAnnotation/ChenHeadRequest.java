...
public class ChenHeadRequest extends ChenBaseRequest {
    public ChenHeadRequest(ChenHttpContext httpContext) {
        super(httpContext);
    }

    @Override
    public ChenResponse request(ChenMethodContext context) throws IOException {
        return get(buildHttp(context));
    }
    ...
    private ChenResponse get(HttpURLConnection connection) throws IOException {
        ...
        if (code < 300) {
            return new ChenResponse(code, null, connection.getHeaderFields());
        } else {
            return new ChenResponse(code, null, connection.getHeaderFields(), connection.getResponseMessage());
        }
    }
}
